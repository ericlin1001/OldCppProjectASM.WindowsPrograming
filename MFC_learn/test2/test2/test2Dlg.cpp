...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT1, m_adda);
	DDX_Control(pDX, IDC_EDIT2, m_addb);
	DDX_Control(pDX, IDC_EDIT3, m_addsum);
}
...
BEGIN_MESSAGE_MAP(Ctest2Dlg, CDialogEx)
	...
	ON_CBN_SELCHANGE(IDC_COMBO1, &Ctest2Dlg::OnCbnSelchangeCombo1)
END_MESSAGE_MAP()
...
{
	CString a,b;
	this->m_adda.GetWindowTextA(a);
	this->m_addb.GetWindowTextA(b);
	double fa,fb,sum;
	fa=atof(a);
	fb=atof(b);
	TRACE("%s,%s",a,b);
	//
	sum=fa+fb;
	TRACE("%f+%f=%f",fa,fb,sum);
	CString str;
	char buffer[100];
	sprintf(buffer,"%f",sum);
	str.SetString(buffer);
	this->m_addsum.SetWindowTextA(buffer);
...
void Ctest2Dlg::OnCbnSelchangeCombo1()
{
	// TODO: Add your control notification handler code here
}
